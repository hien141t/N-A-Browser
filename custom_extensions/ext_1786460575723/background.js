...
function fillFormAndSolveCaptchaI9(username, password, fullname, phone) {
    ...
    const run = async () => {
        ...
        const getCaptchaImg = () => getElementByXpath(xpaths.captchaImg) || document.querySelector('img[src*="captcha"], img.captcha-img');
            ...
            for (let attempt = 1; attempt <= 6; attempt++) {
                ...
                if (imgCaptcha && imgCaptcha.src) {
                    try {
                        ...
                        const base64 = await imageToBase64(imgCaptcha.src);
                        if (base64) {
        ...
        const btnSubmit = getElementByXpath("(//button[@type='submit'])[1]") || 
    ...
    return new Promise((resolve) => {
        ...
        const interval = setInterval(() => {
            ...
            if (passInputs.length >= 2 && passInputs[0].offsetParent !== null) {
                ...
                setTimeout(() => {
                    const btnSubmit = document.querySelector('button[type="submit"]') || document.querySelector('button:contains("Gửi đi")');
...
async function runBankCardFlow(tabId, site, bankData) {
    ...
    const bankUrl = site + "/Account/AddBankCard";
    ...
    try {
        await chrome.scripting.executeScript({
            target: { tabId: tabId },
            func: (data) => {
                console.log("[i9 Content] Điền form liên kết ngân hàng:", data);
            },
            args: [bankData]
        });
        return { success: true };
