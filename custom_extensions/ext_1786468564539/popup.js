    ...
    chrome.storage.local.get(['savedAccounts'], (result) => {
        ...
        list.forEach((acc, i) => {
            html += `<div style="padding: 4px 0; border-bottom: 1px dashed #1f293d; color: #cbd5e1;">
                ...
                <span style="color:#ec4899;">MK Rút:</span> ${acc.withdrawPass || 'f0'}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('copyAllAccBtn')?.addEventListener('click', () => {
        chrome.storage.local.get(['savedAccounts'], (result) => {
            ...
            let text = list.map(a => `TK: ${a.username} | MK: ${a.password} | MK Rút: ${a.withdrawPass || 'f0'} | Tên: ${a.fullname} | SĐT: ${a.phone} | Site: ${a.site}`).join('\n');
    ...
    document.getElementById('exportTxtBtn')?.addEventListener('click', () => {
        chrome.storage.local.get(['savedAccounts'], (result) => {
            ...
            let text = list.map(a => `TK: ${a.username} | MK: ${a.password} | MK Rút: ${a.withdrawPass || 'f0'} | Tên: ${a.fullname} | SĐT: ${a.phone} | NH: ${a.bankName} | STK: ${a.bankAccount} | Site: ${a.site} | Time: ${a.time}`).join('\n');
...
document.getElementById('moneyPassBtn').addEventListener('click', async () => {
    ...
    const fullname = document.getElementById('fullname').value;
    const phone = document.getElementById('phone').value;
    const withdrawPass = document.getElementById('withdrawPass').value || 'f0';
    const bankName = document.getElementById('bankName').value;
    const bankBranch = document.getElementById('bankBranch').value;
    const bankAccount = document.getElementById('bankAccount').value;
    const site = getSelectedSite();
